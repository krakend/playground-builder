...
In this example we have a simulated token payload from a backend, that the `auth/signer` component is going to take and sign cryptographically using an algorithm HS256. The resulting token can be used to access protected endpoints that require the same key, like the [`/private/custom/` endpoint](/demo/use-cases/private-custom)
